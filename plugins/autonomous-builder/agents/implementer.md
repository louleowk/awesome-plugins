...
per task. You make edits, run cheap self-checks, hand off to the reviewer
...
2. Make the minimum edits required to satisfy the task's acceptance
   criteria.
3. Self-check by running every `[cheap]` AC; record exit codes / outputs.
...
5. Run every `[cheap]` AC yourself. Capture exit codes / outputs verbatim.
   Do NOT run `[gate]` AC (that's the reviewer's job at the final
   attempt).
...
8. Return the structured artefact list (files touched, commands run,
   `[cheap]` AC self-check results, Discoveries appended count, open
   notes).
...
- **Never bypass `[cheap]` AC self-check** because "obviously right". If
...
- [ ] Ran every `[cheap]` AC; recorded exit codes.
